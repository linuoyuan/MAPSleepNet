    ...
    with tf.compat.v1.variable_scope(name) as scope:
        ...
        if bias is not None:
            biases = _create_variable(
                "biases",
                [filter_shape[-1]],
                tf.compat.v1.constant_initializer(bias)
            )
...
def fc_1(x, num_in, num_out, name, relu=True):
...
def batch_norm(name, input_var, is_train, decay=0.999, epsilon=1e-5):
    ...
    with tf.compat.v1.variable_scope(name) as scope:
      ...
      batch_mean, batch_var = tf.nn.moments(x=input_var,
                                            axes=axis,
                                            name="moments")
...
def batch_norm_new(name, input_var, is_train, decay=0.999, epsilon=1e-5):
    ...
    with tf.compat.v1.variable_scope(name) as scope:
        ...
        gamma = tf.compat.v1.get_variable('gamma',
                                shape=params_shape,
                                initializer=tf.compat.v1.random_normal_initializer(mean=1.0, stddev=0.002))
        ...
        batch_mean, batch_variance = tf.nn.moments(x=input_var, axes=axis, name='moments')
