...
from deepsleep.utils2 import iterate_minibatches, iterate_batch_seq_minibatches
...
for test_numm in range(1,11,1):
    
    # data_dir='E:/Lab/EOG_Code/infant_sleep - tf20-2/input prepare/true_ouput/child_c4'
    # output_dir='E:/Lab/EOG_Code/infant_sleep - tf20-2/network/model/child_3/'
    path='/home/zhuhangyu/EOG_Code/infan/network/data/CHAT/C4/'
    data_dir = os.path.join(path,str(test_numm)) 
    output_dir='/home/zhuhangyu/EOG_Code/infan/network/model/multi-group/CNN_ATTN_Ts_ADDLOSS/CHAT/C4/10/'
    n_folds=60
    fold_idx=test_numm
    pretrain_epochs=50
    resume=False
    ...
    # s_model.summary()    
    # s_model.load_weights('E:/Lab/EOG_Code/infant_sleep - tf20-2/network/model/multi-group/CNN_GRU/CHILD/C4/10/fold'
    #                      +'1'+'/sequence_learning/s_model_weights.h5')
    ...
    f_model_dir = os.path.join(output_dir, "fold{}".format(fold_idx), 'feature_learning')
    if not os.path.exists(f_model_dir):
        os.makedirs(f_model_dir)
    
    optimizer_f = tf.keras.optimizers.Adam(5e-5)
    ...
    manager_f = tf.train.CheckpointManager(ckpt_f, f_model_dir, max_to_keep=3)
    ...
    if resume:
        if os.path.exists(f_model_dir):
            if manager_f.latest_checkpoint:
                ckpt_f.restore(manager_f.latest_checkpoint)
                print("Restored from {}".format(manager_f.latest_checkpoint))
    ...
    for epoch in range(ckpt_f.step.numpy(), pretrain_epochs+1):
        y_true_train, y_pred_train, train_loss, train_duration, y_prob_train = \
                                f_run_epoch(x_train, y_train, batch_size, training=True)
        ...
        # for pl in range(len(temp_y_prob_train)):
        #     temp_y_prob_train[pl,:,:] = normalize(temp_y_prob_train[pl,:,:], axis=1, norm = 'max')
        ...
        # y_nonseq_val[0:seq_length-1] = y_true_val[0,:]
        # y_nonseq_val[seq_length:] = y_true_val[:,seq_length-1]  
        ...
        # for pl in range(len(temp_y_prob_val)):
        #     temp_y_prob_val[pl,:,:] = normalize(temp_y_prob_val[pl,:,:], axis=1, norm = 'max')
        ...
        if int(ckpt_f.step) % 50 == 0 or int(ckpt_f.step) == pretrain_epochs:
            save_path = manager_f.save()
            print("Saved checkpoint for step {}: {}".format(int(ckpt_f.step), save_path))
        ckpt_f.step.assign_add(1)
        
        if valid_acc > validaccmax:
            validaccmax = valid_acc
            strvalid = str(validaccmax)
            namee = strvalid + '_' + 'weights.h5'
            ssavepath = os.path.join(f_model_dir, namee)    
            f_model.save_weights(ssavepath)
        
    # tf.saved_model.save(f_model, f_model_dir)
    f_save_path = os.path.join(f_model_dir, "f_model_weights.h5")    
    f_model.save_weights(f_save_path)
    print ("Finish training")
    tf.train.list_variables(tf.train.latest_checkpoint(f_model_dir))
    
    
    s_model_dir = os.path.join(output_dir, "fold{}".format(fold_idx), 'sequence_learning')
    if not os.path.exists(s_model_dir):
        os.makedirs(s_model_dir)
    
    # optimizer_1 = tf.keras.optimizers.Adam(3e-5)
    optimizer_1 = tf.keras.optimizers.Adam(1e-6) #for neonate
    # optimizer_1 = tf.keras.optimizers.Adam(5e-5) #for teen
    optimizer_2 = tf.keras.optimizers.Adam(1e-5)
    # optimizer_2 = tf.keras.optimizers.Adam(2e-4) #for neonate
    loss_s = tf.keras.losses.SparseCategoricalCrossentropy()
    
    step_s = tf.Variable(1, name='step', trainable=False)
    ckpt_s = tf.train.Checkpoint(step=step_s, optimizer=optimizer_2, net=s_model)
    manager_s = tf.train.CheckpointManager(ckpt_s, s_model_dir, max_to_keep=3)
    
    # data_loader_2 = NonSeqDataLoader(data_dir, n_folds, fold_idx, n_classes, seq_length)
    # x_train, y_train, x_valid, y_valid = data_loader_2.load_train_data()
    
    validaccmax = 0.65
    
    if resume:
        if os.path.exists(s_model_dir):
            if manager_s.latest_checkpoint:
                ckpt_s.restore(manager_s.latest_checkpoint)
                print("Restored from {}".format(manager_s.latest_checkpoint))
                print ("[{}] Resume sequence-training ...\n".format(datetime.now()))
            else:
                print ("[{}] Start sequence-training ...\n".format(datetime.now()))
    else:
        print ("[{}] Start sequence-training ...\n".format(datetime.now()))
    
      
    
    for epoch in range(ckpt_s.step.numpy(), pretrain_epochs+1):
        y_true_train, y_pred_train, train_loss, train_duration, y_prob_train = \
                                s_run_epoch(x_train, y_train, batch_size, training=True)
        ...
        if int(ckpt_s.step) % 50 == 0 or int(ckpt_s.step) == pretrain_epochs:
        ...
        if valid_acc > validaccmax:
            ...
            strvalid = str(validaccmax)
            namee = strvalid + '_' + 'weights.h5'
            ssavepath = os.path.join(s_model_dir, namee)    
            s_model.save_weights(ssavepath)
        
    s_save_path = os.path.join(s_model_dir, "s_model_weights.h5")    
    s_model.save_weights(s_save_path)
    #tf.saved_model.save(s_model, s_model_dir)
    print ("Finish fine-tuning")       
    tf.train.list_variables(tf.train.latest_checkpoint(s_model_dir))
